...
class ForeignOfficeCountryServiceSpec extends PlaySpec with GuiceOneAppPerSuite {
  ...
  "find all in English" should {
    "return list of countries ordered by name" in new Scenario {
      val found: Seq[Country] = service.findAll()
  ...
  "UK in English" should {
    "keep reference to UK" in new Scenario {
      val found: Seq[Country] = service.findAll().filter(_.code == "GB")
  ...
  "English country list" should {
    "contain aliases from the aliases file" in new Scenario {
      val gbs: Seq[Country] = service.findAll().filter(_.code == "GB")
      gbs.size.mustBe(>(1))
    }
  }
  ...
  "find all in Welsh" should {
    "return list of countries ordered by name" in new Scenario {
      val found: Seq[Country] = service.findAll(welshFlag = true)
  ...
  "UK in Welsh" should {
    "keep reference to UK" in new Scenario {
      val found: Option[Country] = service.find(welshFlag = true, code = "GB")
  ...
  "Welsh country list" should {
    "contain aliases from the aliases file" in new Scenario {
      val gbs: Seq[Country] = service.findAll(welshFlag = true).filter(_.code == "GB")
      gbs.size.mustBe(>(1))
    }

    "merge country lists from gov.wales and wco" in new Scenario {
      val usa: Seq[Country] = service.findAll(welshFlag = true).filter(_.code == "US")
      usa.size.mustBe(> (1))
