...
@Singleton
class AddressLookupController @Inject()(journeyRepository: JourneyRepository,
  ...
  def getWelshContent(journeyData: JourneyDataV2)(implicit request: Request[_]): Boolean = {
    journeyData.welshEnabled && request.cookies.exists(kv => kv.name == "PLAY_LANG" && kv.value == "cy")
  }
  ...
  def select(id: String): Action[AnyContent] = Action.async { implicit req =>
    withFutureJourneyV2(id) { journeyData =>
      ...
      lookupForm(journeyData.config.options.isUkMode).bindFromRequest().fold(
          ...
          handleLookup(id, journeyData, lookup) map {
            ...
            case ResultsList(addresses, firstLookup) =>
              ...
              Some(journeyDataWithProposals) -> requestWithWelshHeader(isWelsh) {
                Ok(select(id, journeyData, selectForm(), Proposals(Some(addresses)), lookupWithFormattedPostcode, firstLookup, isWelsh, isUKMode))
              }
  ...
  def edit(id: String, lookUpPostCode: Option[String]): Action[AnyContent] = Action.async {
    implicit req => withJourneyV2(id) {
      journeyData => {
        ...
        if (isUKMode) {
          (None, requestWithWelshHeader(isWelsh) {
            Ok(uk_mode_edit(id, journeyData, ukEditForm().fill(editAddress), allowedSeqCountries(Seq.empty), isWelsh, isUKMode))
          })
        }
        else {
          val defaultAddress = addressOrEmpty(journeyData.selectedAddress, lookUpPostCode)
          (None, requestWithWelshHeader(isWelsh) {
            Ok(non_uk_mode_edit(id, journeyData, nonUkEditForm().fill(defaultAddress), allowedSeqCountries(countries(isWelsh)), isWelsh = isWelsh, isUKMode = isUKMode))
          })
        }
      }
    }
    ...
    implicit req => withJourneyV2(id) {
      journeyData => {
        ...
        if (isUKMode) {
          ...
          validatedForm.fold(
            errors => (None, requestWithWelshHeader(isWelsh) {
              BadRequest(non_uk_mode_edit(id, journeyData, errors, allowedCountries(countries(isWelsh), journeyData.config.options.allowedCountryCodes), isWelsh = isWelsh, isUKMode = isUKMode))
            }),
            edit => (Some(journeyData.copy(selectedAddress = Some(edit.toConfirmableAddress(id)))), requestWithWelshHeader(isWelsh) {
              Redirect(routes.AddressLookupController.confirm(id))
            })
