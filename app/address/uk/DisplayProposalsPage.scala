...
object DisplayProposalsPage {
  ...
  def showAddressListProposalForm(ix: Int, nameNo: Option[String], postcode: String, continue: Option[String],
    ...
    val updatedDetails = AddressData(cu, false, nameNo, Some(postcode), ar.uprn.map(_.toString), lines, ad.town, None, country.code)
